...
/*******************************************************************************
* Class Name:       File
* File Name:        class.File.php
* Generated:        Sunday, Jun 19, 2016 - 15:17:42 CEST
*  - for Table:     files
...
class File {
	...
	public function select($mID) { // SELECT Function
		...
		$sSQL = "SELECT * FROM files WHERE id = $mID;";
	...
	function update($mID) {
		$sSQL = "UPDATE files SET (id = '$this->id') WHERE id = $mID;";
		$oResult = $this->database->Query($sSQL);
	}
	
	public function delete($mID) {
		$sSQL = "DELETE FROM files WHERE id = $mID;";
		$oResult = $this->database->Query($sSQL);
	}
