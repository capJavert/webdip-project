...
* Generated:        Sunday, Jun 19, 2016 - 15:18:33 CEST
*  - for Table:     users
...
class User {
	...
	public function select($mID) { // SELECT Function
		...
		$sSQL = "SELECT * FROM users WHERE id = $mID;";
	...
	function update($mID) {
		$sSQL = "UPDATE users SET (id = '$this->id') WHERE id = $mID;";
		$oResult = $this->database->Query($sSQL);
	}
	
	public function delete($mID) {
		$sSQL = "DELETE FROM users WHERE id = $mID;";
		$oResult = $this->database->Query($sSQL);
	}
